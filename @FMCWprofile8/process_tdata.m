...
if ~exist(pdir,'dir') % if directory exists
    ...
    if obj.P.overwrite==0
        cont=('Warning, directory exists - overwrite previous processing results? (Y/N)');
    else
        cont='Y';
    end
end
if strcmp(cont,'Y')
    ...
    obj.M.Fs=rd.M.Fs;
    [n5,m5]=size(rd.TDATA);
    P=zeros(maxP,m5,nbatch)*NaN; % initialize psd matrix
    ...
    obj.CPUtime=C2a(I2);
    obj.Tpl=rd.Tpl;
    obj.TWT=rd.TWT(1:n4); % store two-way travel time
else
...
rd=subdivide_daq(rd); % subdivide raw daq files
rd.TDATA=rd.TDATA(1:(n5-20),:); % remove last 20 to fix size
...
r2.P2=P2;r2.T2=T2;r2.C1=C1;r2.fn=fn;
