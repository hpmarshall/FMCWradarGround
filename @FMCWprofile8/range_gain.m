function obj = range_gain(obj)
% apply an automatic gain control
disp('applying range gain')
mP=quantile(obj.PDATA,0.025,2); % take the 2.5% quantile over entire profile
[n1,m1]=size(obj.PDATA); % get size of PDATA
x=1:n1; % index to time-domain samples
mP2 = nonparametric_smooth8(x,mP,x,obj.P2.gain_window); % smooth in time
obj.PDATA=obj.PDATA-mP2*ones(1,m1); % remove 
...
% NOTE: 09/18/08 - now use only finite points to best handle NaN
...
%function NL = inverse_spreading(twt,Pvec,
