...
if obj.P.channel>0
    filename=[obj.data_dir 'file0' num2str(obj.P.files(1)) '.daq']; % filename
    [data, ~, ~, ~, daqinfo] = daqread(filename); % get some info
    Fs=daqinfo.ObjInfo.SampleRate; % sample freq
    ramp=data(:,1); % voltage ramp
else % if ramp only recorded during first file
    filename=[obj.data_dir 'file00.daq']; % use file 00
    [data, ~, ~, ~, daqinfo] = daqread(filename); % get some info
    Fs=daqinfo.ObjInfo.SampleRate; % sample freq    
    ramp=data(:,2);
end
...
for n=1:length(obj.P.files)
    filename=[obj.data_dir 'file0' num2str(obj.P.files(n)) '.daq']; % filename
    [data, ~, abstime, ~, daqinfo] = daqread(filename); % get some info
    Fs=daqinfo.ObjInfo.SampleRate; % sample freq
    if obj.P.channel>0 % if there is a ramp for every file
        ramp=data(:,1); % voltage ramp
        i1=find(ramp>=obj.P.vrange(1) & ramp<=obj.P.vrange(2)); % data in range
        i2=diff(i1); i3=find(i2>5); % find breaks between traces
        S=[i1(i3(1:end-1)+1) i1(i3(2:end))]; % start and end of each trace
    ...
    tdata=data(:,(obj.P.channel+1)); % time domain data - grab specified channel
    ...
    if max(IND)<=length(tdata) % if we have a complete trace
        ...
        filenumber(t1:t2)=obj.P.files(n);
    end
end
%I2=isfinite(TDATA(1,:));
obj.TDATA=TDATA; %(:,I2);
