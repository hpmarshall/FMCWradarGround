...
dinfo  = daq.getDevices; % get info about devicesty
s.Rate=100000;
% add the Analog Input channels and configure
ch=addAnalogInputChannel(s,dinfo(1).ID,[0 1 2 4],'Voltage') % add channels 0 and 1
ch(2).Range=[-1 1];
ch(3).Range=[-1 1];
ch(4).Range=[-1 1];
% add the Analog Output channels and configure
addAnalogOutputChannel(s,dinfo(1).ID, 0, 'Voltage')% add output channel 0

%% still need to add 3rd plot

%% radar specific analog outputft
fullBW=12;lowfreq=6; % oscillator bandwidth and minimum frequency
startfreq=6; stopfreq=18; % start/stop freq for measurement
freqrangemin=6.5;freqrangemax=17.5; % start/stop freq for processing
Fs=s.Rate; % store sample frequency
nfft=2^10; % number of fft points
alpha=2; % fft parameter
ref=1e-5; % reference PSD
v=2.4e10; % velocity rough estimate4ccc
zmin=-100; zmax=-30; % PSD range in dB to plot
depthmin=0;depthmax=150; % min/max depth to plot
ntraces=30000; % max number of traces
