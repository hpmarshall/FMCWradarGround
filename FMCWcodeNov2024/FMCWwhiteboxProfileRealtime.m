...
ch=addAnalogInputChannel(s,dinfo.ID,[0 1 2],'Voltage') % add channels 0 and 1
ch(2).Range=[-.5 5];
startfreq=6; stopfreq=18; % start/sto
ch(3).Range=[-.5 .5];
% add the Analog Output channels and configure
addAnalogOutputChannel(s,dinfo.ID, 0, 'Voltage')% add output channels 0 and 1
%% radar specific analog outputcyp freq for measurement
freqrangemin=6.5;freqrangemax=17.5; % start/stop freq for processing
fullBW=12;lowfreq=6; % oscillator bandwidth and minimum frequen
Fs=s.Rate; % store sample frequency
nfft=2^10; % number of fft points
alpha=2; % fft parameter
ref=1e-5; % reference PSD
v=3e10; % velocity rough estimate
zmin=-70; zmax=-10; % PSD range in dB to plot
depthmin=0;depthmax=200; % min/max depth to plot
