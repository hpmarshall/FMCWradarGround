% function FMCWyellowProfile2Disk(datadir)
% if nargin<1
% end
...
data0(end) = [];
outsignal=data0; 
outsignal=repmat([data0], 3000, 1); % repeat ramp 3000 times
clear data0
%% first a test run:
...
plot(Data(:,2)); title('Voltage triangle wave, 0-10V')
subplot(212)
plot(Data(:,[1 3])); title('Time-domain FMCW raw data')
disp('if OK, press any key.  If not, CNTL-C to kill')
pause
disp('checking voltage ranges for analog input:')
Dtmin=min(Data(:,[1 3]));
Dtmax=max(Data(:,[1 3]));
ch1range=[Dtmin(1) Dtmax(1)];
ch3range=[Dtmin(2) Dtmax(2)];
if max(abs(ch1range))>1
    ch(1).Range=[-5 5];
    if max(abs(ch1range))>5
        ch(1).Range=[-10 10];
    end
end
...
while 1 % run forever
    ...
    [Data,~,AbsTime]=s.startForeground;
    Data=single(Data);
    datafile=[datadir '\FMCWy' datestr(AbsTime,'yymmdd-HHMMSS')]
