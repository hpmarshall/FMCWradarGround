...
Fs=1e5; nsamptrace=5200;
AI=analoginput('nidaq','dev1'); % input object for DAQCARD
chan=addchannel(AI,[0 1 2]); % add 2 channels, ramp, radar input (far from each other)
AI.Channel(1).InputRange=[-10 10]; % set input range for ramp
AI.Channel(2:3).InputRange=[-10 10]; % set input range of radar input
set(AI.Channel(1),'ChannelName','Ramp') % name each channel for displays
set(AI.Channel(2),'ChannelName','I')
set(AI.Channel(2),'ChannelName','Q')
% set(AI,'TriggerType','Manual');
% set(AI,'ManualTriggerHwOn','Trigger')
set(AI,'TriggerConditionValue',0.3); % trigger at 0.1 Volts
...
%set(AI,'TriggerType','Immediate') % trigger on PFI0/TRIG1 - Note this is much better than channel 1
...
%ind=1:length(vdata);
vlow=1; vhigh=9;
ind1=find(vdata>vlow); %((freqrangemin-lowfreq)*10/fullBW));
ind2=find(vdata>vhigh); %((freqrangemax-lowfreq)*10/fullBW));
ind=ind1(1):ind2(1);

% and calculate the psd 
pTpl=length(ind)./Fs; % processed pulse length
BW=9.6; %freqrangemax-freqrangemin; % processed bandwidth
nfft=2^13; alpha=2; ref=1; v=3e10;
% check if we need to decimate
t=0:1/Fs:((nsamptrace-1)/Fs);
if length(ind)>nfft
    fac=ceil(length(ind)/nfft); % factor to decimate by
    tdata=decimate(tdata(ind,:),fac); % decimate the data
    t=decimate(t(ind),fac); % decimate the data
    vdata=decimate(vdata(ind),fac); % decimate the data
...
%tdata=tdata(ind);
ww = KaiserBessel(length(tdata(:,1)),alpha); % calculate window
[psd,w]=CAL_PSD2(tdata(:,1),ww,nfft,Fs); % calculate the windowed, zero-padded FFT
[psd2,w]=CAL_PSD2(tdata(:,2),ww,nfft,Fs); % calculate the windowed
psd_dB=10*log([psd(:) psd2(:)]/ref);
...
%t=0:1/Fs:((nsamptrace-1)/Fs);
hramp=plot(t,vdata);
htitle=title('Trace #: 1');
set(gcf,'doublebuffer','on'); % reduce plot flicker
subplot(3,1,2)
htime=plot(t,tdata); 
set(gcf,'doublebuffer','on'); % reduce plot flicker
subplot(3,1,3)
hfreq=plot(d,psd_dB); hold on
set(gcf,'doublebuffer','on'); % reduce plot flicker
axis([0 500 zmin zmax])
...
%     tdata=tdata-msky;
    ...
    [psd,w]=CAL_PSD2(tdata(:,1),ww,nfft,Fs); % calculate the windowed, zero-padded FFT
    [psd2,w]=CAL_PSD2(tdata(:,2),ww,nfft,Fs); % calculate the win
    psd_dB=10*log([psd(:) psd2(:)]/ref);
    t2={tdata(:,1);tdata(:,2)};
    p2={psd_dB(:,1);psd_dB(:,2)};
    % update plots
    set(hramp,'YData',vdata);
    set(htime,'YData',t2);
    set(hfreq,'YData',p2)
    drawnow
    set(htitle,'string',['Trace #: ' num2str(n)])
    %stop(AI)
    putdata(AO,outsignal')
